...
class PubsubAdhoc(object):
	def __init__(self, pubsub):
		...
		self.adhoc = self.xmpp.plugin['old_0050']
		
		createleaf = self.xmpp.plugin['xep_0004'].makeForm('form', "Create Leaf")
		createleaf.addField('node', 'text-single')
		self.xmpp.plugin['old_0050'].addCommand('newleaf', 'Create Leaf', createleaf, self.createLeafHandler, True)

		#createcollect = self.xmpp.plugin['xep_0004'].makeForm('form', "Create Collection")
		#createcollect.addField('node', 'text-single', 'Node name')
		#self.xmpp.plugin['old_0050'].addCommand('newcollection', 'Create Collection', createcollect, self.createCollectionHandler, True)
		...
		self.xmpp.plugin['old_0050'].addCommand('setitem', 'Set Item', setitem, self.setItemHandler, True)
		...
		self.xmpp.plugin['old_0050'].addCommand('remitem', 'Retract Item', remitem, self.retractItemHandler, True)

		
		confnode = self.xmpp.plugin['xep_0004'].makeForm('form', "Configure Node")
		confnode.addField('node', 'text-single')
		self.xmpp.plugin['old_0050'].addCommand('confnode', 'Configure Node', confnode, self.updateConfigHandler, True)
		...
		self.xmpp.plugin['old_0050'].addCommand('subnode', 'Subscribe Node', subnode, self.subscribeNodeHandler, True)
	
		delnode = self.xmpp.plugin['xep_0004'].makeForm('form', "Delete Node")
		delnode.addField('node', 'text-single')
		self.xmpp.plugin['old_0050'].addCommand('delnode', 'Delete Node', delnode, self.deleteNodeHandler, True)
		...
		self.xmpp.plugin['old_0050'].addCommand('affiliation', 'Change Affiliation', affiliation, self.setAffiliation, True)
